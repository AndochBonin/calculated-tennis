...
func TestAllocator_Allocate_buy(t *testing.T) {
	...
	size, err := a.Allocate(context.Background(), order.SideBuy, decimal.RequireFromString("0.50"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAllocator_Allocate_maxOrderCap(t *testing.T) {
	...
	size, err := a.Allocate(context.Background(), order.SideBuy, decimal.RequireFromString("1"))
	if err != nil {
		t.Fatal(err)
	}
	if !size.Equal(decimal.NewFromInt(10)) {
...
func TestAllocator_Allocate_insufficientBalance(t *testing.T) {
	...
	_, err := a.Allocate(context.Background(), order.SideBuy, decimal.RequireFromString("0.50"))
	if err == nil {
...
func TestAllocator_Allocate_sellRejected(t *testing.T) {
	...
	_, err := a.Allocate(context.Background(), order.SideSell, decimal.RequireFromString("0.50"))
	if err == nil {
		t.Fatal("expected error")
	}
	if status.Code(err) != codes.FailedPrecondition {
